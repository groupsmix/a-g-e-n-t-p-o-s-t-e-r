...
describe('agent-registry', () => {
  ...
  it('every type has a complete descriptor', () => {
    for (const type of AGENT_TASK_TYPES) {
      const desc = getAgent(type)
      ...
      expect(['real', 'stub', 'planned']).toContain(desc!.status)
  ...
  it('listAgents preserves AGENT_TASK_TYPES order', () => {
  ...
  it('getAgent returns null for unknown types', () => {
    expect(getAgent('not-a-real-type')).toBeNull()
  })

  it('listAgentsByStatus filters correctly', () => {
    const real = listAgentsByStatus('real')
    expect(real.length).toBeGreaterThan(0)
    expect(real.every((a) => a.status === 'real')).toBe(true)
    const realTypes = real.map((a) => a.type)
    // research + memory-consolidate are the two "real" handlers shipping in Phase 2/4
    expect(realTypes).toContain('research')
    expect(realTypes).toContain('memory-consolidate')
  })

  it('listAgentsByTag works for common tags', () => {
    const content = listAgentsByTag('content')
