...
const envSchema = z.object({
  ...
  // Audit #44: spend cap. Maximum number of LLM API calls per calendar day
  // (UTC). When the counter hits this limit, further LLM calls are refused
  // until the next day. Default 500 is generous for a single-operator setup;
  // raise it for fleet / multi-tenant deployments.
  MAX_DAILY_LLM_CALLS: z.coerce.number().int().min(0).default(500),
