  ...
  async publish(content: PostContent): Promise<PublishResult> {
    try {
      ...
      if (this.format === "reels" || content.type === "video") {
        ...
        // Audit #26: check HTTP status before trusting the response body.
        if (!containerRes.ok) {
          const body = await containerRes.text().catch(() => "");
          throw new Error(`IG video container HTTP ${containerRes.status}: ${body}`);
        }
        ...
        // Audit #26: check HTTP status before trusting the response body.
        if (!publishRes.ok) {
          throw new Error(`IG video publish HTTP ${publishRes.status}`);
        }
        const published = (await publishRes.json()) as { id?: string };
        if (!published.id) {
          throw new Error("IG video publish returned no id");
        }
      ...
      if (!imageRes.ok) {
        ...
        throw new Error(`IG image container HTTP ${imageRes.status}: ${body}`);
      }
      ...
      // Audit #26: check HTTP status before trusting the response body.
      if (!publishRes.ok) {
        throw new Error(`IG image publish HTTP ${publishRes.status}`);
      }
      const published = (await publishRes.json()) as { id?: string };
      if (!published.id) {
        throw new Error("IG image publish returned no id");
      }
      return this.success(published.id);
    } catch (error) {
