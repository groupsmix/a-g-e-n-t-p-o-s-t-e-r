...
export class ThreadsPublisher extends BasePlatformPublisher {
  ...
  async publish(content: PostContent): Promise<PublishResult> {
    try {
      ...
      // Audit #27: check HTTP status before trusting the response body.
      if (!containerRes.ok) {
        const body = await containerRes.text().catch(() => "");
        throw new Error(`Threads container HTTP ${containerRes.status}: ${body}`);
      }
      ...
      // Audit #27: check HTTP status and assert id before declaring success.
      if (!publishRes.ok) {
        throw new Error(`Threads publish HTTP ${publishRes.status}`);
      }
